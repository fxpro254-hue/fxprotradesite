            ...
            switch (type) {
                ...
                case 'UPDATE_TICK_COUNT':
                    ...
                    if (newCount && !isNaN(newCount)) {
                        ...
                        try {
                            if (window.volatilityAnalyzer && typeof window.volatilityAnalyzer.updateTickCount === 'function') {
        ...
        if (originalUpdateUI) {
            window.updateUI = function() {
                ...
                if (window.tickHistory && window.tickHistory.length > 0) {
                    const currentPrice = window.tickHistory[window.tickHistory.length - 1].quote.toFixed(window.decimalPlaces);
                    window.postMessage({
                        type: 'PRICE_UPDATE',
                        price: currentPrice,
